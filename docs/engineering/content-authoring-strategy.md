...
### Authoring with an agent (phase 5)

§7.1 answered itself: the authors are one person and Claude Code, so the work went into
making *that* pair fast rather than into a CMS. Four things landed for it:

- **`npm run check:content`** — compilers, curriculum invariants and the map in ~4s. The
  full `verify` gate is minutes, which is the wrong loop for a typo in a lesson.
- **`docs/curriculum-map.md`** — the whole curriculum on one page, generated by
  `curriculumMap.test.ts` and kept honest by `toMatchFileSnapshot`. "What do we already
  teach about the serve?" was previously a 79-file read.
- **The level-initial rule is now enforced**, not just observed. It was a convention every
  file followed and nothing checked — exactly the kind of rule an agent breaks.
- **`.claude/skills/authoring-content/`** — the procedure (pick the slot, wire the drills,
  run the check, commit the map) as a skill, and a permission allowlist for the read-only
  commands the loop uses.

None of this changes the pipeline; it changes how quickly a wrong file becomes a red test.
