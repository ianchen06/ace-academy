...
| `npm run check:content` | The content loop: compilers + curriculum invariants, ~4s. See below. |
...
**Start from the map.** `docs/curriculum-map.md` is every lesson, drill and quiz on one
page — ordering prefix, id, title, summary and drill links. Read it before writing to see
what is already covered and which slot is free, rather than opening 79 files. It is
generated by `src/data/curriculumMap.test.ts`: never hand-edit it, and commit the
regenerated file alongside the content that changed, or CI fails on a stale map.

**The loop is `npm run check:content`** (~4s) — the compilers, the curriculum invariants
and the map, without the component tests, Playwright or coverage that a prose edit cannot
affect. Run `npm run verify` once before pushing. The step-by-step procedure for adding an
item lives in the `authoring-content` skill.
...
- **An id carries its level's initial** — `b-` beginner, `i-` intermediate, `a-` advanced —
  and the build fails when it disagrees with the directory the file is in. Every one of the
  79 content files already followed this, but nothing checked it, so moving a file between
  levels without renaming it produced an id that lied about where it lived. Ids are visible
  in URLs and load-bearing in progress keys, so the mismatch is worth a build failure.
